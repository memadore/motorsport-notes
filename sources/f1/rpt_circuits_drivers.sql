select
    ...
    min(
        if(result.classification = 1, _session.session_start_utc::date, null)
    ) as first_win,
    max(
        if(result.classification = 1, _session.session_start_utc::date, null)
    ) as last_win,
    ...
    min(
        if(result.classification <= 3, _session.session_start_utc::date, null)
    ) as first_podium,
    max(
        if(result.classification <= 3, _session.session_start_utc::date, null)
    ) as last_podium,
    ...
    min(
        if(result.classification <= 10, _session.session_start_utc::date, null)
    ) as first_top10,
    max(
        if(result.classification <= 10, _session.session_start_utc::date, null)
    ) as last_top10,
